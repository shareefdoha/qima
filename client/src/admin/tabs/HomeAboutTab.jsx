...
export default function HomeAboutTab() {
  ...
  return <TabPanel title="Home About Section" description="Manage the brief QIMA overview and featured image displayed on the home page.">
    ...
    {!form ? <LoadingBlock label="Loading Home About section…" /> : <form onSubmit={submit} className="max-w-3xl space-y-6 rounded-2xl border border-navy-100 bg-white p-6 shadow-card sm:p-8">
      ...
      <Field label="Featured image" hint="Upload a landscape image (maximum 8 MB). Leave blank to retain the current image.">
        <input type="file" accept="image/*" onChange={(e) => setForm({ ...form, image: e.target.files?.[0] || null })} className="block w-full text-sm text-navy-600 file:mr-4 file:rounded-lg file:border-0 file:bg-navy-100 file:px-4 file:py-2 file:font-medium file:text-navy-800 hover:file:bg-navy-200" />
